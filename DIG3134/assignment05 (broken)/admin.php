      ...
      <?
        if (isset($_SESSION["login"])){
          while ($row = $all_review_result->fetch_object()) {
            ...
            print "<td><img href=\"".$row->game_image_url."\"/></td>"; // .$row->game_image_url.
            print "<td>".date("M d, Y g:ia", strtotime($row->review_creation_date))."</td>";
            print "<td><a href=\"delete.php?review_id =".$row->review_id."\">Delete</a></td>";//doesn't work, should give delete page something to grab onto
            print "</tr>";
          } //while loop
        } //if login
